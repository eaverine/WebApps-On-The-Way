...
def post_share(request, post_id):
    #First retrieve post by id
    post = get_object_or_404(Post, id = post_id, status = Post.Status.PUBLISHED)
    
    #Set variable to check for sent successfully status
    sent = False
    
    if request.method == 'POST':
        #If Submit form button was selected
        form = EmailPostForm(request.POST)
        
        if form.is_valid():
            cd = form.cleaned_data
            
            #Then, build email components and send email
            ...
            send_mail(subject, message, 'oladuaraji@gmail.com', [cd['to']])
            
            #Set variable for successful status
            sent = True
...
def post_comment(request, post_id):
    ...
    if form.is_valid():
        #First create a Comment object without saving it to the database
